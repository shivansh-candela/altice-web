...
class IperfTest(LFCliBase):
    ...
    def generic_cleanup(self, created_cx_cl = None,created_endp_cl=None):
        print("Cleaning up cxs and endpoints")
        if created_cx_cl is None and created_endp_cl is None:
            created_cx_cl  = self.created_cx_cl
            created_endp_cl = self.created_endp_cl
        for cx_name in created_cx_cl:
        ...
        for endp_name in created_endp_cl:
    ...
    def layer3_creation(self, sta_list):
        self.cx_profile.create(endp_type="lf_udp", side_a=self.macvlan_parent, side_b=sta_list, sleep_time=0)
    ...
    def pre_cleanup(self):
        ...
        try:
            ...
            list(map(lambda cx_name: [self.local_realm.rm_endp(ename=i) for i in [f"{cx_name}-A", f"{cx_name}-B"]], exist_l3))
        except Exception as e:
            print("###", e, '###')
        try:
            exist_genric = list(filter(lambda g: True if g != '' else False,
                                       [list(i.values())[0]['name'] for i in self.json_get("/generic/?fields=name")['endpoints']]))
            exist_generic_cx = [f"CX_{i}" for i in exist_genric]
            exist_generic_endp = []
            for ex in exist_genric:
                exist_generic_endp.append(ex)
                exist_generic_endp.append(f"D_{ex}")

            print("Clear generic...")
            self.generic_cleanup(created_cx_cl = exist_generic_cx, created_endp_cl = exist_generic_endp)
        except Exception as e:
            print("###",e,"###")
...
def main():
    parser = LFCliBase.create_bare_argparse(
        ...
        description='''\
1. Run uplink test for 2.4GHz = python3 iperf3_test.py --mgr 192.168.200.12 --mgr_port 8080 --ssid portal --passwd [Blank] --sec open --radio0 wiphy0 --radio1 wiphy1 --macvlan eth1 --mode0 6 --mode1 10 --num_ports 2 --tx_rate 5000 --time 60 --side_a_min_speed 5000000 --side_b_min_speed 500000 --test_2G 1 --test_2G_up 1
2. Run all test for all scenario = python3 iperf3_test.py --mgr 192.168.200.12 --mgr_port 8080 --ssid portal --passwd [Blank] --sec open --radio0 wiphy0 --radio1 wiphy1 --macvlan eth1 --mode0 6 --mode1 10 --num_ports 2 --tx_rate 5000 --time 60 --side_a_min_speed 5000000 --side_b_min_speed 500000 --all_test 1 
3. Run downlink and bi-dir test for 5GHz = python3 iperf3_test.py --mgr 192.168.200.12 --mgr_port 8080 --ssid portal --passwd [Blank] --sec open --radio0 wiphy0 --radio1 wiphy1 --macvlan eth1 --mode0 6 --mode1 10 --num_ports 2 --tx_rate 5000 --time 60 --side_a_min_speed 5000000 --side_b_min_speed 500000 --test_5G 1 --test_5G_dw 1 --test_5G_bi 1
4. Run bi for 2+5 GHz scenario = python3 iperf3_test.py --mgr 192.168.200.12 --mgr_port 8080 --ssid portal --passwd [Blank] --sec open --radio0 wiphy0 --radio1 wiphy1 --macvlan eth1 --mode0 6 --mode1 10 --num_ports 2 --tx_rate 5000 --time 60 --side_a_min_speed 5000000 --side_b_min_speed 500000 --test_both 1 --test_both_bi 1
''')
    parser.add_argument('--ssid', help='SSID for stations to associate')
    ...
    parser.add_argument('--mode0', help='select mode for 2.4 GHz, e.g: 6')
    parser.add_argument('--mode1', help='select mode for 5 GHz, e.g: 10')
    ...
    parser.add_argument('--side_a_min_speed', help='--speed you want to monitor traffic with (max is 10G). side_a which means '
                                               'endpoint-A always connect to stations',
                        default="0")
    parser.add_argument('--side_b_min_speed', help='--speed you want to monitor traffic with (max is 10G). side_b which means '
                                               'endpoint-B always connect to upstream(eth1)',
                        default="10")
    ...
    while True:
        if (num_macvlan < int(num_ports / 2)):
            macvlan_ip_list = ip_test.json_get("/port/1/%s/%s#%s?fields=ip" % (int(args.resource), args.macvlan_parent, num_macvlan))
    ...
    report = lf_report(_results_dir_name = "Iperf3",_output_html="iperf3.html", _output_pdf="iperf3.pdf")
    ...
    def run_scenarios(test, radio_frq, radio1, radio2, mode, up_scenario, dw_scenario, bi_scenario):
        ...
        if (dw_scenario is not None) or (up_scenario is None and dw_scenario is None and bi_scenario is None):
            report.set_obj_html("Download-Single Radio (%s GHz)" % (radio_frq),
                                ...
                                ".X-axis shows the station name and y-axis shows the throughput in Mbps" % (
                                num_ports, radio_frq))
            report.build_objective()
            graph = lf_bar_graph(_data_set=[dw],
                                 _xaxis_name="stations",
                                 _yaxis_name="Throughput (Mbps)",
                                 _xaxis_categories=range(1, num_ports+1,step),
                                 _graph_image_name="downlink_radio_dw_%sGHz" % (radio_frq),
                                 _label=["Downlink"],
                                 _color=[None],
                                 _color_edge='red',
                                 _enable_csv=True,_xticks_font=6,_xaxis_step = step)
        ...
        if (up_scenario is not None) or (up_scenario is None and dw_scenario is None and bi_scenario is None):
            ...
            graph = lf_bar_graph(_data_set=[up],
                                 ...
                                 _color_edge='red',
                                 _enable_csv=True,
                                 _xticks_font=6,_xaxis_step = step)
        ...
        if (bi_scenario is not None) or (up_scenario is None and dw_scenario is None and bi_scenario is None):
            report.set_obj_html("L3-BiDirectional-Single Radio(%s GHz)" % (radio_frq),
                                "The scenerio gives the result of BiDirectional test for %s clients connected on %s "
                                "GHz.X-axis shows the station name and y-axis shows the throughput in Mbps" % (
                                num_ports,
                                radio_frq))
            report.build_objective()
    ...
    summary_data = pd.DataFrame({
        ...
        'Radio': ["2.4 GHz", "2.4 GHz", "2.4 GHz", "5 GHz", "5 GHz", "5 GHz", "2.4 Ghz +5 GHz", "2.4 Ghz +5 GHz",
                  "2.4 Ghz +5 GHz"],
        'Traffic': ["Download", "Upload", "Download,Upload", "Download", "Upload", "Download,Upload", "Download",
        ...
        "Aggregate Throughput(Min)/Client": [str(up_min_value[0]), str(dw_min_value[0]),
                                             "%s Mbps,%s Mbps" % (bi_min_value[0], bi_min_value[1]),
                                             str(up_min_value[1]), str(dw_min_value[1]),
                                             "%s Mbps,%s Mbps" % (bi_min_value[2], bi_min_value[3]),
                                             str(up_min_value[2]),
                                             str(dw_min_value[2]), "%s Mbps,%s Mbps" % (bi_min_value[4], bi_min_value[5])],
        "Aggregate Throughput(Max)/Client": [str(up_max_value[0]), str(dw_max_value[0]),
                                             "%s Mbps,%s Mbps" % (bi_max_value[0], bi_max_value[1]),
                                             str(up_max_value[1]), str(dw_max_value[1]),
                                             "%s Mbps,%s Mbps" % (bi_max_value[2], bi_max_value[3]),
                                             str(up_max_value[2]),
                                             str(dw_max_value[2]), "%s Mbps,%s Mbps" % (bi_max_value[4], bi_max_value[5])],
        "Overall Thrughput": [str(up_all_avg_value[0]), str(dw_all_avg_value[0]),
                              "%s Mbps,%s Mbps" % (bi_all_avg_value[0], bi_all_avg_value[1]),
                              str(up_all_avg_value[1]), str(dw_all_avg_value[1]),
                              "%s Mbps,%s Mbps" % (bi_all_avg_value[2], bi_all_avg_value[3]),
                              str(up_all_avg_value[2]), str(dw_all_avg_value[2]),
                              "%s Mbps,%s Mbps" % (bi_all_avg_value[4], bi_all_avg_value[5])],
        "Aggregate Throughput(Avg)": [str(aggregate[0]), str(aggregate[1]),
                                      "%s Mbps,%s Mbps" % (aggregate[2], aggregate[3]),
                                      str(aggregate[4]), str(aggregate[5]),
                                      "%s Mbps,%s Mbps" % (aggregate[6], aggregate[7]),
                                      str(aggregate[8]), str(aggregate[9]),
                                      "%s Mbps,%s Mbps" % (aggregate[10], aggregate[11])]})
    report.set_obj_html("",
