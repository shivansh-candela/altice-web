"""This script is used to create station one by one and there layer3 traffic one at a time if the station is not connecting at that time it will try to reconnect 3 times and generate traffic
--> date- 08-March-2021
--> Nikita Yadav
"""
import sys
        ...
        if self.local_realm.wait_for_ip([sta_name], timeout_sec=60):
            ...
            if self.local_realm.wait_for_ip([sta_name], timeout_sec=60):
               ...
               return 1
                ...
                if self.local_realm.wait_for_ip([sta_name], timeout_sec=60):
                   self.local_realm._pass("All stations got IPs", print_=True)
                   self.cx_profile.create(endp_type="lf_udp", side_a=self.upstream, side_b=[sta_name],sleep_time=0)
                   self.cx_profile.start_cx()
                   return 1
                else:
                    self.local_realm._fail("Stations failed to get IPs", print_=True)
