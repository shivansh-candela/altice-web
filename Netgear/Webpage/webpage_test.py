...
how to run -
python3 webpage_test.py --mgr 192.168.200.29 --mgr_port 8080 --upstream_port eth1 --num_stations 10 --security open --ssid testap210 --passwd [BLANK] --total_urls 1 --bands 5G --file_size 10MB  --fiveg_radio wiphy0 --duration 2 --threshold_5g 90 --ap_name TESTAP --ssh_port 22
...
class HttpDownload(Realm):
    def __init__(self, lfclient_host, lfclient_port, upstream, num_sta, security, ssid, password, quiesce_after,
                file_size, bands, start_id=0, twog_radio=None, fiveg_radio=None, _debug_on=False, _exit_on_error=False,
                 _exit_on_fail=False):
        ...
        self.target_per_ten = 100
        self.file_size = file_size
        self.bands = bands
        self.debug = _debug_on
        self.quiesce_after = quiesce_after
        print(bands)
        ...
        self.http_profile.quiesce_after = self.quiesce_after
    ...
    def delete_existing_stations_and_layer4_connections(self):
        ...
        exist_sta = list(filter(lambda c: True if 'sta' in c or 'wlan' in c or 'vap' in c else False,
                                [i[list(i.keys())[0]]['alias'] for i in
                                 self.local_realm.json_get("/port/?fields=alias")['interfaces']]))
        if len(exist_sta) != 0:
        ...
        json_data = self.local_realm.json_get("layer4/list?fields=name")
    ...
    def monitor_uc_avg(self):
        ...
        if self.num_sta == 1:
            ...
            for i in range(self.num_sta):
                data1.append((str(list(data['endpoint'][i].keys())))[2:-2])
        data2 = []
        for i in range(self.num_sta):
            if self.num_sta == 1:
    ...
    def monitor_bytes(self):
        ...
        for i in range(self.num_sta):
            if self.num_sta == 1:
                data2.append((data['endpoint']['bytes-rd']))
            else:
                # print(type(data['endpoint'][i][data1[i]]['uc-avg']))
    ...
    def generate_report(self,date, num_stations,duration, test_setup_info,dataset,lis,bands,threshold_2g,threshold_5g,threshold_both,dataset2,summary_table_value,result_data,test_input_infor,total_url):
        ...
        report.set_obj_html("Download Time Graph",
                            "The below graph provides information about the  download time taken by each client to download " + str(total_url) +" webpage urls")
        report.build_objective()
        ...
        report.set_obj_html("Download Rate Graph",
                            "The below graph provides information about the download rate in MBps of each client to download " + str(total_url) +" webpage urls")
        report.build_objective()
        ...
        report.set_obj_html("Download Time Table Description",
                            "This Table will provide you information of the minimum, maximum and the average time taken by clients to download " + str(total_url) + " webpage in seconds")
...
def main():
    ...
    parser.add_argument('--duration', type=int, help='time to run traffic in minutes')
    parser.add_argument('--threshold_5g',help="Enter the threshold value for 5G Pass/Fail criteria in seconds", default="60")
    parser.add_argument('--threshold_2g',help="Enter the threshold value for 2.4G Pass/Fail criteria in seconds", default="90")
    parser.add_argument('--threshold_both',help="Enter the threshold value for Both Pass/Fail criteria in seconds", default="50")
    parser.add_argument('--ap_name', help="specify the ap model ", default="N/A")
    parser.add_argument('--ssh_port', type=int, help="specify the shh port eg 22", default=22)
    parser.add_argument('--total_urls', type=int, help="specify the total urls you want to download", default=1)
    ...
    for bands in args.bands:
        http = HttpDownload(lfclient_host=args.mgr, lfclient_port=args.mgr_port,
                            ...
                            quiesce_after=args.total_urls
                            )
        http.file_create(ssh_port=args.ssh_port)
        http.delete_existing_stations_and_layer4_connections()
        http.set_values()
        http.precleanup()
        http.build()
        time.sleep(1)
    ...
    http1.generate_report(date, num_stations=args.num_stations,
                          ...
                          summary_table_value=summary_table_value, result_data=result_data, test_input_infor=test_input_infor,total_url=args.total_urls)
