...
class IperfTest(LFCliBase):
    def __init__(self, host, port, _local_realm, ssid, security, passwd, radio="wiphy0", macvlan_type = "iperf3_serv", sta_type = "iperf3", num_ports=1, macvlan_parent=None,
                 dhcp=False,port_list=[], sta_list=[],sta_list2=[], _debug_on=False,_exit_on_error=False,_exit_on_fail=False):
    ...
    def create_station(self, radio, sta_list):
    ...
    def create_gen_for_client(self, gen_sta_list, dest, suppress_related_commands_=None):
        ...
        for endp_tpl in endp_tpls:
            ...
            self.cmd = "iperf3 --forceflush --format k --precision 4 -c %s -t 60 --tos 0 -b 1K --bind_dev %s -i 1 " \
                       "--pidfile /tmp/lf_helper_iperf3_%s.pid" % (dest[server_ip], name, gen_name_a)
            data_cmd = {
                "name": gen_name_a,
                "command": self.cmd
            }
...
''')
    ...
    port_list = LFUtils.port_name_series(prefix=args.macvlan_parent + "#", start_id=0,
                                         ...
                                         radio=args.radio)
    station_list_rad0 = LFUtils.port_name_series(prefix="sta" + "#", start_id=0,
                                         end_id=int(num_ports/2) - 1, padding_number=100000,
                                         radio="wiphy0")
    station_list_rad1 = LFUtils.port_name_series(prefix="sta" + "#", start_id=int(num_ports/2),
                                                 end_id=num_ports - 1, padding_number=100000,
                                                 radio="wiphy1")

    ip_test = IperfTest(args.mgr, args.mgr_port, ssid=args.ssid,_local_realm = None,
                        passwd=args.passwd,
                        security=args.security, port_list=port_list,sta_list=station_list_rad0, sta_list2=station_list_rad1, _debug_on=args.debug, macvlan_parent=args.macvlan_parent,
                        dhcp=True, num_ports=args.num_ports)
    ip_test.build()
    ip_test.create_station("wiphy0", station_list_rad0)
    time.sleep(15)
    num_macvlan = 0
    target_ip_rad0 = []
    target_ip_rad1 = []
    ...
    time.sleep(5)
    i=0
    ip_test.create_gen_for_client(station_list_rad0, target_ip_rad0)
    time.sleep(10)
    ip_test.create_station("wiphy1", station_list_rad1)
    time.sleep(15)
    ip_test.create_gen_for_client(station_list_rad1, target_ip_rad1)
    ip_test.macwlan_cx()
    ip_test.generic_cx()
    time.sleep(10)
