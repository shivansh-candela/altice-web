""" This script is used to test the load balancing feature of Netgear AP for 3 categories like channel utilization, rssi test and the max client connect test and after complition of test it will generate a pass/fail report in the html directory
cli - python3 lf_load.py  -i 192.168.208.196 -u root  -p Password@123xzsawq@! -hst localhost -s loadbalance -pwd [BLANK] -sec open -rad wiphy3 --rssi2 35 --rssi5l 32 --rssi5h 30  --client 20  --client5 30 --client5h 35 --ch_threshold 90 --ch_threshold5 80 --ch_threshold5h 90
...
def main():
    ...
    for channel in ["6", "36", "161"]:
        ...
        if value == "not done":
            ...
            if rssi == str(int(rssi_threshold) - 1) or rssi == rssi_threshold or rssi == str(int(rssi_threshold) + 1):
                data = "PASS"
    ...
    summary_table2ghz = [rssi_pass_fail[0], channel_pass_fail[0], max_client_pass_fail[0]]
    summary_table5ghz = [rssi_pass_fail[1], channel_pass_fail[1], max_client_pass_fail[1]]
    summary_table5ghzh = [rssi_pass_fail[2], channel_pass_fail[2], max_client_pass_fail[2]]
