...
class IperfTest(LFCliBase):
    def __init__(self, host, port, ssid, security, passwd,upstream, radio="wiphy0", upstream_port="eth1", macvlan_type = "iperf3_serv", sta_type = "iperf3", num_ports=1, macvlan_parent=None, first_mvlan_ip=None, netmask=None, gateway=None,
                 dhcp=True,port_list=[], sta_list=[], ip_list=None,dest=None, test_duration="5m", interval=1, _debug_on=False,_exit_on_error=False,_exit_on_fail=False):
...
''')
    ...
    if args.first_mvlan_ip is not None:
        ...
        dhcp = True
    #station_list = []
    ip_test = IperfTest(args.mgr, args.mgr_port, ssid=args.ssid,
                        passwd=args.passwd, upstream=args.upstream_port,
                        security=args.security, port_list=port_list, test_duration=args.test_duration, sta_list=None, ip_list=ip_list,
