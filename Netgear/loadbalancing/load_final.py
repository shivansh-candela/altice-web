...
def main():
        ...
        if val == "not done":
            ...
            for i in x:
                ...
                for j in i.split(' '):
                    y.append(j)
            print(y[4])
            max_client_measured.append(y[4])
            if y[4] <= args.client5:
                test = "PASS"
            else:
                test = "FAIL"
    ...
    for channel in ["8", "36", "161"]:
        ...
        time.sleep(10)
        station = Station(lfclient_host=args.host, lfclient_port=8080, ssid=args.ssid, paswd=args.passwd,security=args.security, radio=args.radio, num_sta=1)
