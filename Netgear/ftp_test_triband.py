...
class FtpTest(LFCliBase):
    def __init__(self, lfclient_host="localhost", lfclient_port=8080, sta_prefix="sta", start_id=0, num_sta=None,
                 ...
                 direction=None, duration=None, traffic_duration=None, fivegH_radio=None, fivegL_radio=None, channel_5GH= None,
                 channel_5GL=None, mode=None):
        ...
        self.fivegH_radio = fivegH_radio
        self.fivegL_radio = fivegL_radio
        self.channel_5GH = channel_5GH
        self.channel_5GL = channel_5GL
        self.mode = mode
    ...
    def delete_existing_stations_and_layer4_connections(self):
        '''method for deleting existing stations and layer4 connections'''

        # deleting existing stations
        print("Cleaning Existing stations")
        exist_sta = list(filter(lambda c: True if 'sta' in c or 'wlan' in c or 'vap' in c else False,
                                [i[list(i.keys())[0]]['alias'] for i in
                                 self.json_get("/port/?fields=alias")['interfaces']]))
        if len(exist_sta) != 0:
            self.station_profile.cleanup(desired_stations=exist_sta)
        else:
            print("Existing stations are not available")

        # deleting existing layer4 connections
        print("Cleaning up existing cxs and endpoints")
        json_data = self.json_get("layer4/list?fields=name")
        if 'endpoint' in json_data:

            for i in json_data['endpoint']:
                if isinstance(json_data['endpoint'], list):
                    a = "CX_%s" % list(i.keys())[0]
                    b = list(i.keys())[0]
                elif isinstance(json_data['endpoint'], dict):
                    a = "CX_%s" % json_data['endpoint']['name']
                    b = json_data['endpoint']['name']
        ...
        if self.mode == "dual_band":
            if self.band == "5G":
                ...
                if self.num_sta % 2 == 0:
                    ...
                    raise Exception("Provide even number of stations for Both bands. Because in Both band number of stations are half for 2.4G and half for 5G")
            else:
                raise Exception("Provide proper band name")
        elif self.mode == "tri_band":
            if self.band == "5G-H":
                self.radio = [self.fivegH_radio]
            elif self.band == "5G-L":
                self.radio = [self.fivegL_radio]
            elif self.band == "2.4G":
                self.radio = [self.twog_radio]
            elif self.band == "Tri":
                self.radio = [self.fivegH_radio, self.fivegL_radio, self.twog_radio]
                if self.num_sta % 3 == 0:
                    # if Tri then number of stations will equally divide on triband(5G high, 5G low, 2.4G)
                    self.num_sta = self.num_sta // 3
                else:
                    raise Exception(
                        "Provide proper number of stations which are divisible by 3")
            else:
                raise Exception("Provide proper band name")
    ...
    def precleanup(self):
        ...
        for rad in self.radio:
            if self.mode == "dual_band":
                ...
                if rad == self.twog_radio:
                    self.station_profile.mode = 6
                    self.count = self.count + 1

                elif rad == self.fivegL_radio:
                    self.count = self.count + 1
                    self.channel_selection(self.channel_5GL, rad, 9)

                elif rad == self.fivegH_radio:
                    self.count = self.count + 1
                    self.channel_selection(self.channel_5GH, rad, 9)
            ...
            if self.mode == "dual_band":
                ...
                #Clients are distributed on triband
                if self.count == 3:
                    ...
                    if self.c == 0:
                        # if Both band then first 20 stations will connects to 2.4G
                        self.station_profile.mode = 6
                        self.sta_start_id = self.num_sta
                        self.num_sta = 3 * (self.num_sta)
                        self.station_list1 = LFUtils.portNameSeries(prefix_=self.sta_prefix, start_id_=self.sta_start_id,
                                                                end_id_=(self.sta_start_id * 2) - 1, padding_number_=10000,
                                                                radio=rad)
                        self.station_profile.cleanup(self.station_list1, debug_=self.debug)
                        LFUtils.wait_until_ports_disappear(base_url=self.lfclient_url,
                    ...
                    if self.c == 1:
                        self.station_list2 = LFUtils.portNameSeries(prefix_=self.sta_prefix, start_id_=self.sta_start_id * 2,
                                                                    end_id_=self.num_sta - 1, padding_number_=10000,
                                                                    radio=rad)

                        # cleanup station
                        self.station_profile.cleanup(self.station_list2, debug_=self.debug)
                        LFUtils.wait_until_ports_disappear(base_url=self.lfclient_url,
                                                           port_list=self.station_list2,
                                                           debug=self.debug)
                        self.c = 0
                        return
    ...
    def channel_selection(self, channel_number, radio, profile_mode):
        '''This method for channel selection for tri band'''
        host = self.host
        ...
        lf_r.jsonPost()
        #print("setting radio to channel %d" % channel_number )
        time.sleep(10)
            ...
            if self.mode == "dual_band":
                ...
                if self.count == 3:
                    if self.c == 0:
                        ...
                        self.station_list = self.station_list2
    ...
    def postcleanup(self):
        ...
        LFUtils.wait_until_ports_disappear(base_url=self.lfclient_url, port_list=self.station_profile.station_names,
                                           debug=self.debug)
        if self.mode == "tri_band":
            self.set_default_value_radios()

    def set_default_value_radios(self):
        '''set default value of radios for triband after operation'''
        for i in self.radio:
            self.channel_selection("AUTO", i, 0)
    ...
    def my_monitor(self, time1):
        '''method for monitoring bytes-rd and uc-avg'''
        ...
        self.data1 = []
        if self.num_sta == 1:
            self.data1.append(data['endpoint']['name'])
        else:
        ...
        while str(datetime.datetime.now() - time1) <= self.traffic_duration:
            ...
            while list_of_time.count(0) != 0:
                ...
                for i in range(self.num_sta):
                    ...
                    if self.num_sta == 1:
                        bytes_rd = data['endpoint']['bytes-rd']
                    else:
                        bytes_rd = data['endpoint'][i][data2[i]]['bytes-rd']

                    if bytes_rd <= self.file_size_bytes:
                        data = self.json_get("layer4/list?fields=bytes-rd")
    ...
    def pass_fail_check(self, time_list):
        if (time_list.count(0) == 0) and (max(time_list) < (self.pass_fail_duration * 60)):
            return "Pass"
        else:
            return "Fail"
    ...
    def add_pass_fail_table(self, result_data):
        ...
        i = 0
        table_dict_pass_fail["No. of clients-Band"] = self.rows_head
        for size in self.file_sizes:
    ...
    def download_upload_time_table(self, result_data):
        ...
        string_data = ""
        i = 0
        table_dict_time["No. of clients-Band"] = self.rows_head
    ...
    def generate_graph_time(self,result_data, x_axis, band, size):
        ...
        for data in result_data.values():
            if data["band"] == band and data["file_size"] == size and data["direction"] == "Download":
                ...
                graph_name = "Download time for " + size + " file for " + str(num_stations) + " clients in " + band + " band"
                ...
                graph_description = graph_description + "Out of " + str(data["num_stations"]) + " clients, " + str(
                    data["num_stations"] - fail_count - data["time"].count(0)) + " are able to download " + "within " + str(
                    data["duration"]) + " min."
                count = count + 1
            if data["band"] == band and data["file_size"] == size and data["direction"] == "Upload":
                ...
                graph_name = "Upload time for " + size + " file for " + str(
                    num_stations) + " clients in " + band + " band"
                fail_count = len([i for i in data["time"] if i > (data["duration"] * 60)])
                graph_description = graph_description + "Out of " + str(data["num_stations"]) + " clients, " + str(
                    data["num_stations"] - fail_count - data["time"].count(0)) + " are able to upload " + "within " + str(
                    data["duration"]) + " min."
                count = count + 1
        if count == 2:
            graph_name = "Download and Upload time for " + size + " file for " + str(
                num_stations) + " clients in " + band + " band"
            handles = []
            for i in labels:
                if i == "Upload":
    ...
    def generate_graph_throughput(self, result_data, x_axis, band, size):
        ...
        for data in result_data.values():
            if data["band"] == band and data["file_size"] == size and data["direction"] == "Download":
                ...
                labels.append("Download")
                color.append("Orange")
                graph_name = "Download throughput for " + size + " file for " + str(
                    num_stations) + " clients in " + band + " band"
                graph_description = graph_description + str(data["num_stations"] - data["time"].count(0)) + " clients are able to download " + data["file_size"] + " file."
                count = count + 1
            if data["band"] == band and data["file_size"] == size and data["direction"] == "Upload":
                dataset.append(data["throughput"])
                labels.append("Upload")
                color.append("Blue")
                graph_name = "Upload throughput for " + size + " file for " + str(
                    num_stations) + " clients in " + band + " band"
                ...
                count = count + 1
        if count == 2:
            graph_name = "Download and Upload throughput for " + size + " file for " + str(
                num_stations) + " clients in " + band + " band"
...
def main():
    ...
    optional.add_argument('--mgr', help='hostname for where LANforge GUI is running: eg: localhost , default localhost', default='localhost')
    optional.add_argument('--mgr_port', help='port LANforge GUI HTTP service is running on: eg: 8080 , default 8080', default=8080)
    optional.add_argument('--upstream_port', help='non-station port that generates traffic: eg: eth1', default='eth1')
    required.add_argument('--ssid', type=str, help='--ssid')
    required.add_argument('--passwd', type=str, help='--passwd')
    required.add_argument('--security', type=str, help='--security eg: open|wep|wpa|wpa2|wpa3')
    required.add_argument('--ap_name', type=str, help='--ap_name')
    required.add_argument('--ap_ip', type=str, help='--ap_ip')
    required.add_argument('--channel_5GL', type=int, help='Specify channel number 5G low band')
    required.add_argument('--channel_5GH', type=int, help='Specify channel number 5G high band')
    optional.add_argument('--mode', type=str, help='specify the mode eg: dual_band or tri_band', default='dual_band')
    ...
    optional.add_argument('--fivegH_radio', type=str, help='specify radio for 5G high client', default='wiphy0')
    optional.add_argument('--fivegL_radio', type=str, help='specify radio for 5G low client', default='wiphy2')
    optional.add_argument('--twog_duration', nargs="+", help='Pass and Fail duration for 2.4G band in minutes')
    optional.add_argument('--fiveg_duration', nargs="+", help='Pass and Fail duration for 5G band in minutes')
    optional.add_argument('--Both_duration', nargs="+", help='Pass and Fail duration for Both band in minutes')
    optional.add_argument('--fivegH_duration', nargs="+", help='Pass and Fail duration for 5G high band in minutes')
    optional.add_argument('--fivegL_duration', nargs="+", help='Pass and Fail duration for 5G low band in minutes')
    optional.add_argument('--Tri_duration', nargs="+", help='Pass and Fail duration for Tri band in minutes')
    optional.add_argument('--twog_traffic_duration', nargs="+", help='duration for layer 4 traffic running in minutes for 2.4G band')
    optional.add_argument('--fiveg_traffic_duration', nargs="+", help='duration for layer 4 traffic running in minutes for 5G band')
    optional.add_argument('--Both_traffic_duration', nargs="+", help='duration for layer 4 traffic running in minutes for Both band')
    optional.add_argument('--fivegH_traffic_duration', nargs="+", help='duration for layer 4 traffic running in minutes for 5G high band')
    optional.add_argument('--fivegL_traffic_duration', nargs="+", help='duration for layer 4 traffic running in minutes for 5G low band')
    optional.add_argument('--Tri_traffic_duration', nargs="+", help='duration for layer 4 traffic running in minutes for Tri band')


    # Test variables
    optional.add_argument('--bands', nargs="+", help='--bands defaults ["5G","2.4G","Both"] for dual band and for tri band ["5G-H","5G-L","2.4G","Tri"]',
                        default=["5G", "2.4G", "Both"])
    optional.add_argument('--directions', nargs="+", help='--directions defaults ["Download","Upload"]',
                        default=["Download", "Upload"])
    optional.add_argument('--file_sizes', nargs="+", help='--File Size defaults ["2MB","500MB","1000MB"]',
                        default=["2MB", "500MB", "1000MB"])
    optional.add_argument('--num_stations', type=int, help='--num_stations is number of stations', default=40)
    ...
    def pass_fail_duration(band, file_size):
        '''Method for set pass/fail duration according file size and band which are given by user'''
        ...
        if band == "2.4G":
            ...
            if len(args.file_sizes) is not len(args.Both_duration):
                raise Exception("Give proper Pass or Fail duration for Both band")
            for size in args.file_sizes:
                if size == file_size:
                    index = list(args.file_sizes).index(size)
                    duration = args.Both_duration[index]
        elif band == "5G-H":
            if len(args.file_sizes) is not len(args.fivegH_duration):
                raise Exception("Give proper Pass or Fail duration for 5G high band")
            for size in args.file_sizes:
                if size == file_size:
                    index = list(args.file_sizes).index(size)
                    duration = args.fivegH_duration[index]
        elif band == "5G-L":
            if len(args.file_sizes) is not len(args.fivegL_duration):
                raise Exception("Give proper Pass or Fail duration for 5G low band")
            for size in args.file_sizes:
                if size == file_size:
                    index = list(args.file_sizes).index(size)
                    duration = args.fivegL_duration[index]
        elif band == "Tri":
            if len(args.file_sizes) is not len(args.Tri_duration):
                raise Exception("Give proper Pass or Fail duration for Tri band")
            for size in args.file_sizes:
                if size == file_size:
                    index = list(args.file_sizes).index(size)
                    duration = args.Tri_duration[index]
        ...
        return duration
    def traffic_duration(band, file_size):
        '''Method for set traffic duration according file size and band which are given by user'''

        if band == "2.4G":
            if len(args.file_sizes) is not len(args.twog_traffic_duration):
                raise Exception("Give proper traffic duration for 2.4G band")

            for size in args.file_sizes:
                if size == file_size:
                    index = list(args.file_sizes).index(size)
                    duration = args.twog_traffic_duration[index]
        elif band == "5G":
            if len(args.file_sizes) is not len(args.fiveg_traffic_duration):
                raise Exception("Give proper traffic duration for 5G band")
            for size in args.file_sizes:
                if size == file_size:
                    index = list(args.file_sizes).index(size)
                    duration = args.fiveg_traffic_duration[index]
        elif band == "Both":
            if len(args.file_sizes) is not len(args.Both_traffic_duration):
                raise Exception("Give proper traffic duration for Both band")
            for size in args.file_sizes:
                if size == file_size:
                    index = list(args.file_sizes).index(size)
                    duration = args.Both_traffic_duration[index]
        elif band == "5G-H":
            if len(args.file_sizes) is not len(args.fivegH_traffic_duration):
                raise Exception("Give proper Pass or Fail duration for 5G high band")
            for size in args.file_sizes:
                if size == file_size:
                    index = list(args.file_sizes).index(size)
                    duration = args.fivegH_traffic_duration[index]
        elif band == "5G-L":
            if len(args.file_sizes) is not len(args.fivegL_traffic_duration):
                raise Exception("Give proper Pass or Fail duration for 5G low band")
            for size in args.file_sizes:
                if size == file_size:
                    index = list(args.file_sizes).index(size)
                    duration = args.fivegL_traffic_duration[index]
        elif band == "Tri":
            if len(args.file_sizes) is not len(args.Tri_traffic_duration):
                raise Exception("Give proper Pass or Fail duration for Tri band")
            for size in args.file_sizes:
                if size == file_size:
                    index = list(args.file_sizes).index(size)
                    duration = args.Tri_traffic_duration[index]
        if duration.isdigit():
            duration = int(duration)
        else:
            duration = float(duration)
        ...
        for direction in args.directions:
            for file_size in args.file_sizes:
                ...
                obj = FtpTest(lfclient_host=args.mgr,
                              ...
                              traffic_duration=traffic_duration(band, file_size),
                              fivegH_radio=args.fivegH_radio,
                              fivegL_radio=args.fivegL_radio,
                              mode=args.mode,
                              channel_5GL=args.channel_5GL,
                              channel_5GH=args.channel_5GH
                              )

                iteraration_num = iteraration_num + 1
                if iteraration_num == 1:
                    obj.delete_existing_stations_and_layer4_connections()
                obj.file_create()
                obj.set_values()
                obj.precleanup()
                obj.build()
