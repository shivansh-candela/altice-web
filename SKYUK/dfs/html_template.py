...
def radar_detect_description(data= " This Table will give you results in YES or NO if the AP detects the Radar and 'N/A' - When channel never set in AP "):
    test_radar = """
                    <!-- Radar Detect status -->
...
def client_connection_detail(data = "This Table will give you results in seconds which is measured value of the time taken by the client to connect and generate traffic after Radar detection 'N/A' - when you cannot set a channel in AP and connect time is not applicable '-' when radar is not detected  'X' - when radar detected but channel not switched "):
    test_client = """
...
def switched_channel_details(data = "This Table will give you result value of channel number to which the client switches after radar detection  'N/A' - When channel is not assigned in AP and channel switch not applicable 'X' - radar detected but AP switched to AUTO mode or the switching never occured  '-'  Radar not detected so switching channel is not applicable"):
    switch_channel = """
                                <!-- switched channe;l -->
    ...
    for fcc in list(result_data.keys()):
        ...
        for i in fcc_type:
            if i == "YES":
                ...
                final_data = final_data + "<td colspan='1' bgcolor='yellow'>N/A</td>"
            else:
...
def generate_report(result_data=None,
    ...
    # print("date", date)
    # print("test setup", test_setup_info)
    # print("input", input_setup_info)
    # print("test cha", test_channel)
