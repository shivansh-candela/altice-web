...
def banner_data(date):
    banner_data = """
                       <!DOCTYPE html>
                        ...
                        <title>LANforge Channel Test Report</title>                        

                        </head>

                        <title>CHANNEL SELECTION TEST</title></head>
                        <body>
                        ...
                        <br>
                        <h1 class='TitleFontPrint' style='color:darkgreen;'>  CHANNEL SELECTION TEST  </h1>
...
def test_objective(objective='The Channel Selection Test is designed to check the performance of the Access Point, '
                  'When AP is turned on 2.4 radio will automatically set to a channel, it is found that '
                  ' 60% of the AP’s are selecting channel 11, 15-20% AP’s are working on channel 6,  15-20% AP’s are setting on channel 1. '
                  'This test major objective is to check what are the factors which is causing AP switch automatically . '):
    test_objective = """
                    <!-- Test Objective -->
...
def generate_graph1(expected_value, measured_value,graph_path):
    ...
    return str(graph_html(graph_path=graph_path + "/channel.png", graph_name="Channel Test Graph"))

def detailed_result(final_data):
...
"""
    ...
    if "Traffic1" in x:
        col_head_list.append("channel 1 Throughput(Mbps)")
    if 'Traffic2' in x :
        col_head_list.append("channel 6 Throughput(Mbps)")
    if 'Traffic3' in x :
        col_head_list.append("Channel 11 Throughput(Mbps)")
    ...
    html_data = """
                                       <table border="1" width="1000px" cellpadding="2" cellspacing="0">
                                        ...
                                        <!-- Table information -->
                                        <p align='left' width='900'>This Table will provide you information of the measured channel after reboot and also the expected channel value for every throughput combination.</p>
