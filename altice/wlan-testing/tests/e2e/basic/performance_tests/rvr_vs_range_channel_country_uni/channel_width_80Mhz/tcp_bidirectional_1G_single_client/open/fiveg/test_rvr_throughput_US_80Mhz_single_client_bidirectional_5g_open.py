...
pytestmark = [pytest.mark.country_code, pytest.mark.nat, pytest.mark.open, pytest.mark.united_states,
              pytest.mark.bandwidth_80mhz, pytest.mark.al, pytest.mark.tcp, pytest.mark.rate_vs_range,
              pytest.mark.bidirectional, pytest.mark.tcp_bidirectional,
              pytest.mark.rate_vs_range_open_80mhz_all_channels_single_client_bidirectional_1gbps,
              pytest.mark.rate_vs_range_throughput_open_80mhz_all_channels_single_client_bidirectional_1gbps, pytest.mark.fiveg, pytest.mark.tcp]
...
class TestCountryUS80Mhz5G(object):
    ...
    def test_client_open_ch36_80Mhz_US_5g_tcp_bidirectional(self, instantiate_profile, get_lf_logs,
                                                                  lf_test, update_report,
                                                                  station_names_fiveg, lf_tools,
                                                                  test_cases, testbed, al_1, get_configuration,
                                                                  create_lanforge_chamberview_dut, get_attenuators):
        """
           pytest -m "country_code and eightyMhz and open and fiveg and channel149"
        """
        ...
        val = [['modes: 802.11an-AX'], ['pkts: MTU'], ['directions: DUT Transmit;DUT Receive'], ['traffic_types:TCP'],
               ['bandw_options: AUTO'], ['spatial_streams: AUTO'], [f'attenuator: {attenuator}'],
               [f'attenuations: {main_attenuations2}'], ['chamber: 0'], ['tt_deg: 0']]

        if station:
            rvr_o = lf_test.ratevsrange(station_name=station_names_fiveg, mode=mode, download_rate="100%",upload_rate="100%",
                                        duration='60000',
                                        instance_name="RVR_Channel_36_80_Mhz_Tcp_Fiveg_Mode",
                    ...
                    if len(failed_throughput) >= 1:
                        ...
                        if os.path.exists(pdf):
                            ...
                            allure.attach.file(source=pdf,
                                               name="Rate_Vs_Range_1GBPS_bidirectional_Throughput_TCP_80Mhz_open_5g_Test",
                                               attachment_type="PDF")
    ...
    @pytest.mark.tcp_bidirectional
    def test_client_open_ch149_80Mhz_US_5g_tcp_bidirectional(self, instantiate_profile, get_lf_logs,
                                                                  lf_test, update_report,
                                                                  station_names_fiveg, lf_tools,
        ...
        if station:
            rvr_o = lf_test.ratevsrange(station_name=station_names_fiveg, mode=mode, download_rate="100%",upload_rate="100%",
                                        duration='60000',
                                        instance_name="RVR_Channel_149_80_Mhz_Tcp_Fiveg_Mode",
                                        vlan_id=vlan, dut_name=lf_tools.dut_name, raw_lines=val, ssid_channel=channel)
            report_name = rvr_o.report_name[0]['LAST']["response"].split(":::")[1].split("/")[-1]
            print("report name ", report_name)
            entries = os.listdir("../reports/" + report_name + '/')
