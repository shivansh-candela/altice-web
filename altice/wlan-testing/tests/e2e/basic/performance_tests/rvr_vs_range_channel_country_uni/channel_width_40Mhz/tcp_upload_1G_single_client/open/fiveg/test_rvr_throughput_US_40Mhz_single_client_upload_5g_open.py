...
pytestmark = [pytest.mark.country_code, pytest.mark.nat, pytest.mark.open, pytest.mark.united_states,
              ...
              pytest.mark.rate_vs_range_throughput_open_40mhz_all_channels_single_client_upload_1gbps, pytest.mark.fiveg, pytest.mark.tcp]
...
class TestCountryUS40Mhz5G(object):
    ...
    def test_client_open_ch36_40Mhz_US_5g_tcp_upload(self, instantiate_profile, get_lf_logs,
        ...
        if station:
            rvr_o = lf_test.ratevsrange(station_name=station_names_fiveg, mode=mode, download_rate="100%",
                                        duration='60000',
                                        instance_name="Mode_RVR_Channel_36_40_Mhz_Fiveg_Mode",
    ...
    @pytest.mark.tcp_upload
    def test_client_open_ch149_40Mhz_US_5g_tcp_upload(self, instantiate_profile, get_lf_logs,
                                                                  lf_test, update_report,
                                                                  station_names_fiveg, lf_tools,
        ...
        if station:
            rvr_o = lf_test.ratevsrange(station_name=station_names_fiveg, mode=mode, download_rate="100%",
                                        duration='60000',
                                        instance_name="Mode_RVR_Channel_149_40_Mhz_Fiveg_Mode",
                                        vlan_id=vlan, dut_name=lf_tools.dut_name, raw_lines=val, ssid_channel=channel)
            report_name = rvr_o.report_name[0]['LAST']["response"].split(":::")[1].split("/")[-1]
            print("report name ", report_name)
            entries = os.listdir("../reports/" + report_name + '/')
