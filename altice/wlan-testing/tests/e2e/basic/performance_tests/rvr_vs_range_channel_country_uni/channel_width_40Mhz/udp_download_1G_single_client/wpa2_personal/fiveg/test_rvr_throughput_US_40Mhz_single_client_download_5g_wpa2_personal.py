...
pytestmark = [pytest.mark.country_code, pytest.mark.nat, pytest.mark.wpa2, pytest.mark.united_states,
              ...
              pytest.mark.vload, pytest.mark.udp_download,
              pytest.mark.rate_vs_range_wpa2_40mhz_all_channels_single_client_download_1gbps,
              pytest.mark.rate_vs_range_throughput_wpa2_40mhz_all_channels_single_client_download_1gbps, pytest.mark.fiveg, pytest.mark.udp]
...
class TestCountryUS40Mhz5G(object):
    ...
    def test_client_wpa2_ch36_40Mhz_US_5g_udp_download(self, instantiate_profile, get_lf_logs,
        ...
        if station:
            rvr_o = lf_test.ratevsrange(station_name=station_names_fiveg, mode=mode, download_rate="100%",
                                        duration='60000',
                                        instance_name="RVR_Channel_36_40_Mhz_Udp_Fiveg_Mode",
    ...
    @pytest.mark.udp_download
    def test_client_wpa2_ch149_40Mhz_US_5g_udp_download(self, instantiate_profile, get_lf_logs,
                                                                  lf_test, update_report,
                                                                  station_names_fiveg, lf_tools,
        ...
        if station:
            rvr_o = lf_test.ratevsrange(station_name=station_names_fiveg, mode=mode, download_rate="100%",
                                        duration='60000',
                                        instance_name="RVR_Channel_149_40_Mhz_Udp_Fiveg_Mode",
                                        vlan_id=vlan, dut_name=lf_tools.dut_name, raw_lines=val, ssid_channel=channel)
            report_name = rvr_o.report_name[0]['LAST']["response"].split(":::")[1].split("/")[-1]
            print("report name ", report_name)
            entries = os.listdir("../reports/" + report_name + '/')
