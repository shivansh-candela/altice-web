...
pytestmark = [pytest.mark.country_code, pytest.mark.nat, pytest.mark.open, pytest.mark.united_states,
              ...
              pytest.mark.throughput_open_20mhz_all_channels_single_client_bidirectional_1gbps, pytest.mark.udp]
...
class TestCountryUS20Mhz2g(object):
    ...
    def test_client_nat_open_chn1_20Mhz_US_2g_udp_bidirectional(self, instantiate_profile, get_lf_logs,
        ...
        wct_obj = lf_test.wifi_capacity(instance_name="test_client_open_NAT_udp_bd_2g", mode=mode, vlan_id=vlan,
        ...
        if expected_throughput < float(actual_throughput):
            ...
            if os.path.exists(pdf):
                allure.attach.file(source=pdf,
                                   name="WiFi_Capacity_1GBPS_Bidirectional_Throughput_Testt_UDP_2g", attachment_type="PDF")
            assert True
        else:
            result["result"] = "FAIL"
            pdf = lf_tools.create_dynamic_pdf(report_name, get_configuration, result)
            ...
            if os.path.exists(pdf):
                allure.attach.file(source=pdf,
                                   name="WiFi_Capacity_1GBPS_Bidirectional_Throughput_Test_UDP_2g", attachment_type="PDF")
            assert False
    ...
    @pytest.mark.udp_bidirectional
    def test_client_nat_open_chn6_20Mhz_US_2g_udp_bidirectional(self, instantiate_profile, get_lf_logs,
                                              lf_test, update_report,
                                              station_names_twog, lf_tools,
                                              test_cases, testbed, al_1, get_configuration, get_attenuators):
        """
           pytest -m "country_code and twentyMhz and open and twog and channel6"
        """
        profile_data = setup_params_general["ssid_modes"]["open"][0]
        ssid_name = profile_data["ssid_name"]
        security_key = profile_data["security_key"]
    ...
    @pytest.mark.udp_bidirectional
    def test_client_nat_open_chn11_20Mhz_US_2g_udp_bidirectional(self, instantiate_profile, get_lf_logs,
                                              lf_test, update_report,
                                              station_names_twog, lf_tools,
                                              test_cases, testbed, al_1, get_configuration, get_attenuators):
        """
