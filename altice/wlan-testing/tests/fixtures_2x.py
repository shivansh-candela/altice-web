...
class Fixtures_2x:
    ...
    # #
    # #     config = json.loads(
    # #         str(instantiate_profile_obj.base_profile_config).replace(" ", "").replace("'", '"').replace("True", "true"))
    # #     config["uuid"] = 0
    # #
    ...
    def setup_profiles_generic(self, request, param, get_markers, get_configuration, get_apnos,
                               run_lf=False):
        if run_lf:
            return 0
        vlan_id, mode = 0, 0
        ...
        ap_ssh = get_apnos(get_configuration['access_point'][0], pwd="../libs/apnos/",
                           sdk="2.x")  # Get ssh/serial object

        output = ap_ssh.run_generic_command(cmd="")  # To put AP in cli mode
        print("Ap in mode: ", output)

        try:
            if output[0].strip() != "/cli>":
                if output[1].strip() != "/cli>":
                    output = ap_ssh.run_generic_command(cmd="cli")  # To put AP in cli mode
        ...
        for mode in profile_data['ssid']:
            if mode == "open":
                for j in profile_data["ssid"][mode]:
                    if mode in get_markers.keys() and get_markers[mode]:
                        try:
                            if j["appliedRadios"].__contains__("2G"): # This works for 2G
                                wireless_ssid_show_0 = ap_ssh.run_generic_command(
                                    cmd="/wireless/basic/show --wifi-index=0")
                                for i in range(len(wireless_ssid_show_0)):
                                    if wireless_ssid_show_0[i].startswith("** wifi-ssid:"):
                                        available_ssid_in_ap = wireless_ssid_show_0[i]
                                        available_ssid_in_ap = available_ssid_in_ap.split()
                                        expected_ssid = parameter["ssid_modes"]["open"][0]["ssid_name"]
                                        print("Expected SSID: ", expected_ssid)
                                        print("Current SSID: ", available_ssid_in_ap[-1])
                                        if available_ssid_in_ap[-1] != expected_ssid:
                                            cmd = "/wireless/basic/config --wifi-index=0 --wifi-ssid=" + expected_ssid
                                            push_ssid_to_ap = ap_ssh.run_generic_command(cmd=cmd)
                                            # print(push_ssid_to_ap)
                                        else:
                                            print("AP Broadcasting Expected SSID")
                                        wireless_ssid_show_0 = ap_ssh.run_generic_command(
                                            cmd="/wireless/basic/show --wifi-index=0")
                                        allure.attach(name="/wireless/basic/config --wifi-index=0: ",
                                                      body=str(wireless_ssid_show_0))
                                        break
                                wireless_sec_show_0 = ap_ssh.run_generic_command(
                                    cmd="/wireless/security/show --wifi-index=0")
                                # print(wireless_sec_show_0)
                                for i in range(len(wireless_sec_show_0)):
                                    if wireless_sec_show_0[i].startswith("** wifi-wl-auth-mode:"):
                                        available_sec_in_ap = wireless_sec_show_0[i]
                                        available_sec_in_ap = available_sec_in_ap.split()
                                        print("Current SSID sec: ", available_sec_in_ap[-1])
                                        if available_sec_in_ap[-1] != "None":
                                            cmd = "/wireless/security/config --wifi-index=0 --wifi-sec-choose-interface=0 --wifi-wl-auth-mode=None"
                                            push_sec_to_ap = ap_ssh.run_generic_command(cmd=cmd)
                                            # print(push_sec_to_ap)
                                        else:
                                            print("AP Broadcasting Expected security")
                                        wireless_sec_show_0 = ap_ssh.run_generic_command(
                                            cmd="/wireless/security/show --wifi-index=0")
                                        allure.attach(name="/wireless/security/show --wifi-index=0: ",
                                                      body=str(wireless_sec_show_0))
                                        break
                            if j["appliedRadios"].__contains__("5G"): # This works for 5G
                                wireless_ssid_show_1 = ap_ssh.run_generic_command(cmd="/wireless/basic/show --wifi-index=1")
                                for i in range(len(wireless_ssid_show_1)):
                                    if wireless_ssid_show_1[i].startswith("** wifi-ssid:"):
                                        available_ssid_in_ap = wireless_ssid_show_1[i]
                                        available_ssid_in_ap = available_ssid_in_ap.split()
                                        expected_ssid = parameter["ssid_modes"]["open"][1]["ssid_name"]
                                        print("Expected SSID: ", expected_ssid)
                                        print("Current SSID in AP: ", available_ssid_in_ap[-1])
                                        if available_ssid_in_ap[-1] != expected_ssid:
                                            cmd = "/wireless/basic/config --wifi-index=1 --wifi-ssid=" + expected_ssid
                                            push_ssid_to_ap = ap_ssh.run_generic_command(cmd=cmd)
                                            print(push_ssid_to_ap)
                                        else:
                                            print("AP Broadcasting Expected SSID")
                                        wireless_ssid_show_1 = ap_ssh.run_generic_command(
                                            cmd="/wireless/basic/show --wifi-index=1")
                                        allure.attach(name="/wireless/basic/config --wifi-index=1: ",
                                                      body=str(wireless_ssid_show_1))
                                        break
                                wireless_sec_show_1 = ap_ssh.run_generic_command(cmd="/wireless/security/show --wifi-index=1")
                                # print(wireless_sec_show_1)
                                print("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++")
                                for i in range(len(wireless_sec_show_1)):
                                    if wireless_sec_show_1[i].startswith("** wifi-wl-auth-mode:"):
                                        available_sec_in_ap = wireless_sec_show_1[i]
                                        available_sec_in_ap = available_sec_in_ap.split()
                                        print("Current SSID sec: ", available_sec_in_ap[-1])
                                        if available_sec_in_ap[-1] != "None":
                                            cmd = "/wireless/security/config --wifi-index=1 --wifi-sec-choose-interface=0 --wifi-wl-auth-mode=None"
                                            push_sec_to_ap = ap_ssh.run_generic_command(cmd=cmd)
                                            print(push_sec_to_ap)
                                        else:
                                            print("AP Broadcasting Expected security")
                                        wireless_sec_show_1 = ap_ssh.run_generic_command(
                                            cmd="/wireless/security/show --wifi-index=1")
                                        allure.attach(name="/wireless/security/show --wifi-index=1: ",
                                                      body=str(wireless_sec_show_1))
                                        break
                            j["appliedRadios"] = list(set(j["appliedRadios"]))
                            j['security'] = 'none'
                            # creates_profile = instantiate_profile_obj.add_ssid(ssid_data=j)
    ...
    #
    #         config = json.loads(
    #             str(instantiate_profile_obj.base_profile_config).replace(" ", "").replace("'", '"').replace("True",
    #                                                                                                         "true"))
    #         config["uuid"] = 0
    #
    #         # Attach the config that is sent from API
